...
//the following function will take a source string and copy it to a destination string while converting all characters to uppercase. it will iterate through each character in the source string, convert it to uppercase using the toupper function, and store it in the destination string. finally, it will null-terminate the destination string
...
//the following function will check to see if the permissions in the config file allow for the operation in the trace file. it will iterate through each character in the trace operation, and check to see if it is present in the config permissions. if any character in the trace operation is not present in the config permissions, the function will return 0 (false). if all characters in the trace operation are present in the config permissions, the function will return 1 (true)
